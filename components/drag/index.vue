...
<script>
	export default {
		...
		props: {
			...
			containerSize: {
				type: Object,
				default: () => ({ wdith: '100%', height: '100vh' }),
			},
			// 控件的大小
			controlsSize: {
				type: Object,
		...
		methods: {
			...
			initControlsPosition() {
				...
				let tempArray = [];
				// 每行最大存放的个数
				this.maxWidthCount = parseInt(this.systemInfo.windowWidth *2 / (this.controlsSize.width + this.margin.margin_x));

				// 设置控件位置 - 这边多记录一个位置 之后会用到
				for(let i = 0, j = 0, k = 0; i < this.controlsList.length + 1; i++) {
					tempArray[i] = {
						left: j * (this.controlsSize.width + this.margin.margin_x) + (j == 0 ? 0 : this.margin.margin_x),
